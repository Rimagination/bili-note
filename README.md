...
- 完整归档：保存完整字幕、图文正文、图片、完整评论、元数据和证据索引，主笔记中的关键判断可以回到原文位置复核。
...
一次完整提取会生成两层结果：面向阅读的主笔记，以及面向复核和追问的原始材料包。主笔记强调学习收获、关键概念、方法流程、实践清单和证据位置；材料包保存完整字幕或图文正文、图片、评论、元数据、JSONL 索引、预算和评分结果。
...
- 关键判断尽量标注图文证据 `O<opus_id>-E001`、字幕证据 `Pxx@hh:mm:ss-hh:mm:ss` 或评论证据 `C<rpid>`，方便回查完整原文。
